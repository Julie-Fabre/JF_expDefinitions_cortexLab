function oneSideChoiceWorldJF(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
preStimulusPeriod = p.preStimulusDelay.map(@rnd.sample).at(evts.newTrial); % at(evts.newTrial) fix for rig pre-delay 
prestimQuiescentPeriod = at(preStimulusPeriod,evts.newTrial.delay(0)); 
preStimQuiescence = sig.quiescenceWatch(prestimQuiescentPeriod, t, wheel, 10); 
...
% choose an interactive delay period from a uniform distribution (changes
% based on training stage)
waitStage = p.waitStage;
interactiveDelay = cond(waitStage == 0, 0.1, ...
    waitStage == 1, map(evts.newTrial, @(x) unifrnd(0.1,0.5,1)), ...
    waitStage == 2, map(evts.newTrial, @(x) unifrnd(0.3,0.9,1)), ...
    waitStage >= 3, map(evts.newTrial, @(x) unifrnd(0.6,1,1)));
...
interactiveQuiescence = sig.quiescenceWatch(interactiveQuiescentPeriod, t, wheel, 20); 
%% step 4. go!
...
%interactiveOn = sig.quiescenceWatch(interactiveDelay, t, wheel, 10);%$stimulusOn.delay(interactiveDelay); % the closed-loop period starts when the stimulus comes on, plus an 'interactive delay'
...
dispThreshold = iff(p.stimulusOrientation == 45, dispThreshold / 2, dispThreshold);
...
evts.preStimulusDelay = preStimulusPeriod;
...
c = [1 0.5 0.25 0.12 0.06 0];
C = [c, zeros(1, numel(c)-1); zeros(1, numel(c)-1), zeros(1, numel(c))];
%%%
orientationSet = [0, 0, 0];
...
p.waitOnEarlyResponse=true;
p.quiescenceThreshold = 20;
p.waitStage=3;
catch % ex
