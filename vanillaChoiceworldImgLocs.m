function vanillaChoiceworldImgLocs(t, events, parameters, visStim, inputs, outputs, audio)
...
startingContrasts = [true,false,false,false,false,false];
...
onsetToneAmplitude = 0; % changed from 0.3 we don't wanmt cue for one side ; Changed from 1 when moving to Strix
onsetToneFreq = 12000;
...
trialDataInit.trialSide = 1;%randsample([-1,1],1);
...
switch current_min_contrast
    ...
    case 0.5
        % Lower from 0.5 contrast after > 2000% correct (= impossible)
        min_hit_percentage = 20; % 20 so should never happen - hacky way 
