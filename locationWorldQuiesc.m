function locationWorldQuiesc(t, events, parameters, visStim, inputs, outputs, audio)
% 2021-02-24 - AP
%
...
timeout = parameters.timeout.at(events.expStart);
% ITI
itiMin = parameters.itiMin;
itiMax = parameters.itiMax;

%quiescenceTimes 
quiescMin = parameters.quiescMin;
quiescMax = parameters.quiescMax;
%% Fixed parameters
...
try
    ...
    parameters.itiMin = 1;
    parameters.itiMax = 3;
    parameters.quiescMin = 1;
    parameters.quiescMax = 3;
...
stimName = trialDataInit.stimName; 
responseTimeOver = trialDataInit.responseTimeOver; 
%% Set up wheel 
...
%prestimQuiescentTime = response.delay(mapn(quiescMin,quiescMax,@chooseITI));
prestimQuiescentPeriod = at(mapn(quiescMin,quiescMax,@chooseITI),events.newTrial.delay(0)); 
...
responseTimeOver = (t - t.at(stimOn)) > timeout;
response = keepWhen(interactiveOn.setTrigger(abs(stimDisplacement) ...
    >= responseDisplacement | responseTimeOver),interactiveOn.to(events.newTrial));


% responseTimeOver = (t - t.at(stimOn)) > 5; % p.responseWindow may be set to Inf
% threshold = stimOn.setTrigger(...
%   abs(stimDisplacement) >= abs(responseDisplacement) | responseTimeOver);
% response = cond(...
...
% response = response.at(stimOn.setTrigger(threshold)); 

% responseTimeOver = (t - t.at(stimOn)) > 10;
% response = keepWhen(interactiveOn.setTrigger(abs(stimDisplacement) ...
%     >= responseDisplacement| responseTimeOver),interactiveOn.to(events.newTrial)) ;
 % p.responseWindow may be set to Inf
...
% Play noise on miss
audio.default = missNoiseSamples.at(~trialData.hit.delay(0.01));
...
stimAzimuth = cond( ...
    ...
    startingAzimuth*trialData.trialSide.at(interactiveOn) +...
    sign(stimDisplacement.at(response))*responseDisplacement);
...
stim = vis.image(t); 
stim.sourceImage = stimName.at(stimOn);
% Stim flicker
...
% imgDir = '\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\';
% num = randi(7,[1,1]);
% thisim = clown;
% %imgArr = mapn(thisim, imread(fullfile(imgDir, ['img', num2str(num), '.jpeg'])));
% 
% stim = vis.image(t);
% stim.sourceImage = map(imread(fullfile(imgDir, ['img', num2str(num), '.jpeg'])));
stim.dims = [90,90];
...
trialDataInit.stimName = ['zserver.cortexlab.net\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img4.jpeg']; %QQ
trialDataInit.responseTimeOver = 0;
end
...
trialData.trialSide = 1; %chnged from  randsample([-1,1],1) to be one sided
clown = imread(['\\zserver.cortexlab.net\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img4.jpeg']);
trialData.stimName = clown;
end
