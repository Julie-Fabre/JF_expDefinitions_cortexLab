function advancedChoiceWorldJulie_WIP(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
preStimulusDelay = p.preStimulusDelay.map(@rnd.sample).at(evts.newTrial); % at(evts.newTrial) fix for rig pre-delay 
stimulusOn = preStimulusDelay.setEpochTrigger(t, wheel,  p.quiescenceThreshold);
...
ITI = response.delay(iff(response == 0 & p.stimulusOrientation ~= 45, 3, 1));%*itiHit + trialData.miss.at(response)*itiMiss);
...
evts.endTrial = nextCondition.at(stimulusOff).delay(ITI); %p.interTrialDelay.map(@rnd.sample)
