function choiceWorldrewNoGoJFWIP3(t, events, p, visStim, inputs, outputs, audio)
...
orientationSet = p.orientationSet.at(events.expStart);
startingOrientations = p.startingOrientations.at(events.expStart); 
orientationSt = startingOrientations(1); 
...
rewardKeyPressed = inputs.keyboard.strcmp(rewardKey); % true each time the reward key is pressed
responseWindow = p.responseWindow; 
% Sounds
audioDevice = audio.Devices('default');%QQ
...
[trialDataInit] = events.expStart.mapn(...
    contrastSet, startingContrasts, orientationSet, startingOrientations, repeatOnMiss, ...
...
%trialOrientation = iff( events.trialNum < 2,trialDataInit.trialOrientation,trialOrientation);
...
% disp('wtf')
% yy=exist('trialData.useContrasts') || exist('events.wheelGain');
% disp(yy)
% if yy==true
%     disp(trialOrientation)
%     responseWindow = iff(trialOrientation == 45, 1, 60);
...
% else
%disp(exist('trialOrientation'))

%responseWindow2 = iff(trialOrientation == 45, 1, 5);


    responseTimeOver = (t - t.at(stimOn)) > 10; % p.responseWindow may be set to Inf
threshold = stimOn.setTrigger(...
  abs(stimDisplacement) >= abs(p.responseDisplacement) | responseTimeOver);
response = cond(...
    responseTimeOver, 3,... % if the response time is over the response = 0
...
trialData = responseData.at(response).scan(@updateTrialData, trialDataInit).subscriptable;
    responseTimeOver = (t - t.at(stimOn)) > 5; % p.responseWindow may be set to Inf
threshold = stimOn.setTrigger(...
  abs(stimDisplacement) >= abs(p.responseDisplacement) | responseTimeOver);
response = cond(...
    responseTimeOver, 3,... % if the response time is over the response = 0
...
trialContrast = trialData.trialContrast.at(events.newTrial);
hit = trialData.hit.at(response); 
trialOrientation = trialData.trialOrientation.at(events.newTrial);



%orientationSt = trialOrientation; 
%% Task disengagement
...
reward = merge(rewardKeyPressed, hit==true);% only update when feedback changes to greater than 0, or reward key is pressed
outputs.reward = rewardSize.at(reward.delay(0.01)); % output reward
...
stim.orientation = trialOrientation; 
...
events.orientationSet = trialData.orientationSet; %QQ
...
events.orientationLeft = iff(trialData.trialSide == -1, trialData.trialOrientation, trialData.trialOrientation*0);
events.orientationRight = iff(trialData.trialSide == 1, trialData.trialOrientation, trialData.trialOrientation*0);
...
events.useOrientations = trialData.useOrientations;
...
p.orientationSet = [-45, 0.1, 45]'; %QQ 2021/02/05
p.startingOrientations = double([true,true,true]'); %QQ 2021/02/05
...
p.itiMiss = 2; % (seconds)
p.responseWindow = 10; % (seconds)
p.noGoTime = 1;%QQ 2021/02/05, make this longer and longer, add beep, gain change. 
...
function [trialDataInit] = initializeTrialData(expRef, ...
    contrastSet,startingContrasts,orientationSet, startingOrientations, repeatOnMiss,trialsToBuffer, ...
    trialsToZeroContrast,rewardSize)%,responseWindow)
...
startingOrientations = logical(startingOrientations)';
...
if length(expRef) > 1
    ...
    % Loop through blocks from latest to oldest, if any have the relevant
    % parameters then carry them over
    if contains(subject, 'default')
        endCheckE = 200;
    else
        endCheckE = 1;
    end
    for check_expt = length(expRef)-1:-1:endCheckE
        previousBlockFilename = dat.expFilePath(expRef{check_expt}, 'block', 'master');
        if exist(previousBlockFilename,'file')
            previousBlock = load(previousBlockFilename);
        ...
        if exist('previousBlock','var') && all(isfield(previousBlock, ...
                {'useContrastsValues','useOrientationsValues','hitBufferValues','trialsToZeroContrastValues'})) &&...
...
if useOldParams
    ...
    try
      ...
      len2 = length(previousBlock.orientationSetValues)/length(previousBlock.orientationSetTimes);
      trialDataInit.orientationSet = previousBlock.orientationSetValues(end-len2+1:end);
    catch
      len = length(contrastSet');
      trialDataInit.contrastSet = contrastSet';
      len2 = length(orientationSet');
      trialDataInit.orientationSet = orientationSet';
    end
    trialDataInit.useContrasts = previousBlock.useContrastsValues(end-len+1:end);
    trialDataInit.useOrientations = previousBlock.useOrientationsValues(end-len2+1:end);
    % The buffer to judge recent performance for adding contrasts
    trialDataInit.hitBuffer = ...
        previousBlock.hitBufferValues(:,end-len+1:end,:);%QQ need to change for oris. 
...
%     end
    ...
    trialDataInit.orientationSet = orientationSet';
    trialDataInit.useOrientations = startingOrientations;
...
% Set the first contrast %QQ check here for wierd stuff (!!!) (repeat on
% miss only after certain number of trials?, or maz repeat on miss at
% first? 
...
%disp(w)
%disp(contrasts)
orientations = trialDataInit.orientationSet(trialDataInit.useOrientations);
if length(orientations) > 1
    w2 = ((orientations~=0) + 1) / length(unique(orientations));
    trialDataInit.trialOrientation = randsample(orientations, 1, true, w2);

else
    w2 = ((orientations~=0) + 1) / length(unique(orientations));
    trialDataInit.trialOrientation = orientations;

end

%disp(w2)
%disp(orientations)
%trialDataInit.responseWindow = iff(trialDataInit.trialOrientation == 45, 1, 60);
trialDataInit.trialSide = iff(rand <= 0.5, -1, 1);
trialDataInit.responseWindow = 10;
end
...
currentOrientationIdx = trialData.trialOrientation == trialData.orientationSet;
...
%current_min_contrast = min(trialData.contrastSet(trialData.useContrasts & trialData.contrastSet ~= 0)); % change to add azimuth 
current_orientations = sum(trialData.useOrientations);% length(unique(trialData.orientationSet(trialData.useOrientations & (trialData.orientationSet ~= 0))));
%disp(current_orientations)
trialsToBuffer = size(trialData.hitBuffer,1);
switch current_orientations
    
    case 1
        %% need to change: 3 days straight > 70%, move to no-go. 3 days straight no go and stim > 0.7, move to 3rd stimuli. then all contrasts. 
        % Lower from 0.5 contrast after > 70% correct
        min_hit_percentage = 0.8;%changd from 0.7 - i don't want contrasts added 
        ...
        %disp(contrast_total_trials)
        %disp(trialData.hitBuffer)
        if sum(contrast_total_trials) >= size(trialData.hitBuffer,1)
            ...
            if all(use_hits >= min_hits)
                trialData.useOrientations(find(~trialData.useOrientations,1)) = true; 
                %trialData.useContrasts(find(~trialData.useContrasts,1)) = true;
            end
        end

    case 2%%3 %inside this, add case current_contrast min thing 
        % Lower from 0.25 contrast after > 50% correct
        min_hit_percentage = 1;%changed from 0.7
    ...
    case 0.125
        ...
        min_hit_percentage = 1;%changed from 0.65
...
orientations = trialData.orientationSet(trialData.useOrientations);
%w2 = ((orientations~=0) + 1) / length(unique([orientations]));
%trialData.trialOrientation = randsample(contrasts, 1, true, w2);
if length(orientations) > 1
    w2 = ((orientations~=0) + 1) / length(unique(orientations));
    trialData.trialOrientation = randsample(orientations, 1, true, w2);

else
    w2 = ((orientations~=0) + 1) / length(unique(orientations));
    trialData.trialOrientation = orientations;

end
%%%% Pick next side
trialData.trialSide = iff(rand <= 0.5, -1, 1);
trialData.responseWindow = 5;
end
