function advancedChoiceWorldJulieAllStims2(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
%stimulusOn = preStimulusDelay.setEpochTrigger(t, wheel, 90);
stimulusOn = at(true, preStimulusDelay);
...
gain = iff(p.stimulusOrientation == 45, p.noGoWheelGain, p.wheelGain);
stimulusDisplacement = gain*(wheel - wheelOrigin); % yoke the stimulus displacment to the wheel movement during closed loop
stimDispGainNorm = (wheel - wheelOrigin);%QQ
%% define response and response threshold
responseWindow = iff(p.stimulusOrientation == 45, p.noGoResponseWindow, p.responseWindow);
responseTimeOver = (t - t.at(interactiveOn)) > responseWindow; % p.responseWindow may be set to Inf
dispThreshold = abs(p.stimulusAzimuth);
dispThreshold = iff(p.stimulusOrientation == 45, p.dispThresholdNoGo, dispThreshold);
threshold = interactiveOn.setTrigger(...
  abs(stimulusDisplacement) >= dispThreshold | responseTimeOver);
...
correctResponse = cond(p.stimulusOrientation == 45, 0,...
    contrastLeft > contrastRight, -1,... % contrast left
...
azimuth = cond(...
    ...
    threshold.to(stimulusOff),  -response*abs(dispThreshold)); % Once threshold is reached the stimulus is fixed again

%% define the visual stimulus

% Test stim left

% clown = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img1.jpeg']);
% stim_id_str = contrastLeft.map(@num2str);
% disp(stim_id_str)
% stim_image = stim_id_str.map( ...
%   @(curr_str_id) imread(fullfile(imgDir, ['img' curr_str_id '.jpeg'])));
...
clown = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img1.jpeg']);
clown2 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img2.jpeg']);
clown3 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img3.jpeg']);
clown4 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img4.jpeg']);
clown5 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img5.jpeg']);
clown6 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img6.jpeg']);
clown7 = imread(['\\zserver\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img7.jpeg']);
% 
 flipFreq = 0.01; % Switch image at 0.5 HZ
 flip =  skipRepeats(t*flipFreq > 0);%iff(feedback > 0 , 1, 0); % Value of 0 or 1
...
leftStimulus = vis.image(t);
leftStimulus.sourceImage = index.selectFrom(clown, clown2);
leftStimulus.azimuth = -p.stimulusAzimuth + azimuth;
leftStimulus.dims = [70,50];
...
leftStimulus.show = iff(contrastLeft> 0, stimulusOn.to(stimulusOff), false);

vs.leftStimulus = leftStimulus; % store stimulus in visual stimuli set and log as 'leftStimulus'

rightStimulus = vis.image(t); 
rightStimulus.sourceImage = clown;
%stim.sourceImage = stim_image.map(@rescale);
rightStimulus.azimuth = p.stimulusAzimuth + azimuth;
rightStimulus.dims = [70,50];
%leftStimulus = vis.grating(t, 'square', 'gaussian'); % create a Gabor grating
% leftStimulus.orientation = p.stimulusOrientation;
% leftStimulus.altitude = 0;
% leftStimulus.sigma = [20,20]; % in visual degrees
...
% leftStimulus.azimuth = -p.stimulusAzimuth + azimuth;
% When show is true, the stimulus is visible
%rightStimulus.contrast = 1;
rightStimulus.show = iff(contrastRight> 0, stimulusOn.to(stimulusOff), false);
...
nextCondition = feedback > 0 | p.repeatIncorrect == false ; %QQ need to set a max to this 
...
evts.orientation = p.stimulusOrientation;
evts.responseWindow = responseWindow;
evts.dispThresh = dispThreshold;
evts.gain = gain; 
evts.wheelVelGainNorm = stimDispGainNorm; 
...
itiT = iff(correctResponse, p.itiHit, p.itiMiss);
evts.endTrial = nextCondition.at(stimulusOff).delay(itiT); 
...
%%%
orientationSet = [0, 45,-45];
%spatialFrequencySet = []; 
%p.imageType = %whether and which natural image to use 
p.stimulusOrientation = repelem(orientationSet, length(C));
C = repmat(C, 1, length(orientationSet));
p.stimulusContrast = C;
p.repeatIncorrect = abs(diff(C,1)) > 0.25; % | all(C==0);
%p.shortResponse = 1;
p.onsetToneFrequency = 12000;
p.interactiveDelay = 0.4;
p.onsetToneAmplitude = 0.3;
p.responseWindow = 60;
p.stimulusAzimuth = 90;
p.noiseBurstAmp = 0.03;
p.noiseBurstDur = 0.5;
p.rewardSize = 2.5;
p.rewardKey = 'r';
p.spatialFrequency = 1/15; % Prusky & Douglas, 2004
p.interTrialDelay = 0.5;%QQ ITI HIT ITI MISS
p.wheelGain = 5;
p.noGoWheelGain = 1;
p.preStimulusDelay = [0.5 0.5 0.5]';
p.noGoResponseWindow = 1;
p.dispThresholdNoGo = 40;
p.itiHit = 1;
p.itiMiss = 3;
catch % ex
