function advancedChoiceWorldJulie2(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
stimulusOn = sig.quiescenceWatch(preStimulusDelay, t, wheel, 90);
...
ITI = response.delay(iff((response == 0 & p.stimulusOrientation ~= 45) | ...
  (response == -1 & contrastLeft < contrastRight) | ...
  (response == 1 & contrastLeft > contrastRight)|...
  (response ~= 0 & p.stimulusOrientation == 45), p.ITImiss, p.ITIhit));%*itiHit + trialData.miss.at(response)*itiMiss);
