function vanillaChoiceworldfml(t, events, parameters, visStim, inputs, outputs, audio)
...
numba = stimOn; % returns a signal
r= randi(7,[1, 1]);
numba =  r * numba;
%disNumba = randi(7,[1, 1]) * numba;
stim_id_str = numba.map(@num2str);
disp(stim_id_str);
imgDir = '\\zserver.cortexlab.net\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\';
%stim_image = stim_id_str.map(imread(fullfile(imgDir, ['img' stim_id_str '.jpeg'])));

 clown = imread(['\\zserver.cortexlab.net\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img6.jpeg']);

stim = vis.image(t); 
stim.sourceImage = stimName;
% Stim flicker
...
trialDataInit.stimName = ['zserver.cortexlab.net\Data\pregenerated_textures\JulieF\choiceWorld_7Stims\img6.jpeg'];
end
