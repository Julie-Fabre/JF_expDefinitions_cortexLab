function advancedChoiceWorldJulieOriAndSpFzym4(t, evts, p, vs, in, out, audio)
%% advancedChoiceWorld
...
interactiveOn = stimulusOn.delay(interactiveDelay); % the closed-loop period starts when the stimulus comes on, plus an 'interactive delay'
 % the closed-loop period starts when the stimulus comes on, plus an 'interactive delay'
...
leftStimulus.spatialFreq = iff(p.stimulusOrientation == 0, 1/15, 1/7); % in cylces per degree
...
rightStimulus.spatialFreq = iff(p.stimulusOrientation == 0, 1/15, 1/7);
