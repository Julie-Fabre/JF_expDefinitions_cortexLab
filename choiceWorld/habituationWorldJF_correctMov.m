...
function habituationWorldJF_correctMov(t, evts, p, vs, in, out, audio)
%% habituationWorld
...
quiescThreshold = 1;
prestimQuiescentTime = 0;
wheelGain = 2;
responseDisplacement = 5;
%% Audio onset tone
...
response = keepWhen(interactiveOn.setTrigger(abs(stimDisplacement) ...
    >= responseDisplacement),interactiveOn.to(evts.newTrial));

%% feedback
reward = iff(p.useWheel, wheelDelta > p.movementThreshold |...
    t - t.at(evts.newTrial.delay(0)) > map(p.avgRewardTime, @timeSampler),... % movement threshold reached
...
aziUpdate = cond(azimuth < 0 ,))
azi =     cond( ...
        evts.newTrial.to(interactiveOn), startingAzimuth*trialSide, ...
        interactiveOn.to(response), startingAzimuth*trialSide + stimDisplacement, ...
        response.to(evts.newTrial), ...
        startingAzimuth*trialSide.at(interactiveOn) + sign(stimDisplacement.at(response))*responseDisplacement);

stimulus.azimuth = aziUpdate;
...
try
  ...
  % Random rewards
  p.useWheel = false;
  p.movementThreshold = 100; % Irrelevant when useWheel is false
