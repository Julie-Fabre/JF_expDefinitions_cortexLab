function vanillaChoiceworldKilotrodeJF(t, events, parameters, visStim, inputs, outputs, audio)
...
onsetToneAmplitude = 1;%1; %QQ Changed from 1 when moving to Strix
onsetToneFreq = 12000;
...
missNoiseDuration = 0.5; %0.5;
missNoiseAmplitude = 0.05; % QQChanged from 0.05 when moving to Strix
missNoiseSamples = missNoiseAmplitude*events.expStart.map(@(x) ...
    randn(2, audioSampleRate*missNoiseDuration));

% Wheel parameters
quiescThreshold = 1;
wheelGain = 10; % deg/mm should be 8 with corect re setting
...
wheel = inputs.wheelMM.skipRepeats();%in mm diam, pull reso from hardware.mat (1024!)

%% Trial event times
% (this is set up to be independent of trial condition, that way the trial
